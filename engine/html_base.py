#!/usr/bin/env python3
"""BẢN SAO trong repo engine — NGUỒN: Rheinmir/setup `fdk/tools/html_base.py` (framework gác lệch bằng html-font-lint --parity). Đừng sửa ở đây:
sửa ở framework rồi chép đè. Engine cần bản riêng vì nó chạy được không cần overstack.
...
def apply(html: str, *, toggle: bool = True, fix=None) -> str:
    ...
    html = html[:ho.end()] + f'<script id="{BOOT_ID}">{boot}</script>' + html[ho.end():]
