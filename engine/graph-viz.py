...
# `__engine_dir__` do SHIM bên overstack đặt (khi đó `__file__` là đường shim); chạy thẳng thì là thư mục của chính file này.
_ENGINE_DIR = Path(globals().get("__engine_dir__") or Path(__file__).resolve().parent)
KIND_REGISTRY_PATH = _ENGINE_DIR / "kind-glyphs.json"        # nguồn vĩnh viễn — sửa qua PR vào repo orca-graph
# kind MỚI tự sinh giữ ở nhà máy-local, KHÔNG ghi vào thư mục engine (đó là git clone — ghi vào là bẩn cây, chặn update)
KIND_LOCAL_PATH = Path(os.environ.get("ORCA_GRAPH_HOME") or Path.home() / ".orca-graph") / "kind-glyphs.local.json"
...
def svg(g: dict, pos: dict, scale: float = 1.0, mini: bool = False, registry: dict = None, new_kinds: dict = None) -> str:
    ...
    for i, n in nodes.items():
        ...
        for d in n["deps"]:
            ...
            # PRD v1.1 §23.2: reason_class của cạnh. preference = chỉ do thứ tự viết (chấm thưa, mờ); control/acceptance = cổng (màu hổ phách).
            ...
            why = f" — {reason}" if reason else (" — suy luận" if dash else " — chưa khai lý do")
            out.append(f'<path class="{cls}" d="M{ax} {ay} C{cx} {ay} {cx} {by} {bx} {by}"{dash} marker-end="url(#arw)" data-e="{d}->{i}" data-reason="{html.escape(reason)}"><title>{d} → {i} (phụ thuộc{why})</title></path>')
...
def legend_html() -> str:
    return '<div class="legend">' + "".join(f'<span><i style="background:{c}"></i>{html.escape(STATE_VI[s])}</span>' for s, c in STATE_COLOR.items()) + \
           ...
           '<span><i style="background:transparent;border-top:2px dotted var(--edge);height:0"></i>chấm thưa = cạnh preference (chỉ do thứ tự viết — audit-edges có thể đề xuất bỏ)</span>' \
           '<span><i style="background:transparent;border-top:2px solid #d97706;height:0"></i>hổ phách = cổng control/acceptance</span>' \
