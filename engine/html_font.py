#!/usr/bin/env python3
"""BẢN SAO trong repo engine — NGUỒN: Rheinmir/setup `fdk/tools/html_font.py` (framework gác lệch bằng html-font-lint). Đừng sửa ở đây:
sửa ở framework rồi chép đè cả `html_font.py` + `html_font_data.py`. Engine cần bản riêng vì nó chạy được không cần overstack.
