...
raw_install(){ # không git (hoặc bản cài cũ kiểu raw): kéo đúng 4 file engine — chạy được, nhưng không có `git log`, tests/, evals/
  ...
  mkdir -p "$DEST/engine"
  for f in orca-graph.py graph-viz.py graph-atlas.py kind-glyphs.json html_font.py html_font_data.py html_base.py LexendDeca-NOTICE.txt; do curl -fsSL "$RAW/engine/$f" -o "$DEST/engine/$f"; done
}
